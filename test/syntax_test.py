...
from src.proto_syntax import ProtoSyntaxTypes


class SyntaxTest(unittest.TestCase):
    def test_correct_syntax(self):
        self.assertEqual(
            Parser.loads("syntax = 'proto3'").syntax, ProtoSyntaxTypes.PROTO3
        )
        self.assertEqual(
            Parser.loads('syntax = "proto3"').syntax, ProtoSyntaxTypes.PROTO3
        )
        self.assertEqual(
            Parser.loads("syntax = 'proto2'").syntax, ProtoSyntaxTypes.PROTO2
        )
        self.assertEqual(
            Parser.loads('syntax = "proto2"').syntax, ProtoSyntaxTypes.PROTO2
        )
